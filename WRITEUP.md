...
This project builds an end-to-end credit stress-testing framework that connects borrower-level default risk to macroeconomic stress. Using public LendingClub data, the engine estimates 24-month probability of default, calibrates the model to observed portfolio performance, estimates a macro-to-loss transmission channel, and applies historical recession paths to produce stressed default and expected-loss estimates.
...
| 3. Scenario Engine | `03_scenario_engine.ipynb`; refactored deployment: `main.py` + `src/scenario_engine.py` | Transmit historical unemployment stress paths through the satellite model to produce stressed default and expected-loss estimates. |
...
### 5.4 Implementation and AWS Deployment

Component 3 was refactored from the original notebook workflow into a reusable Python scenario engine with `main.py` as the entry point. The refactored implementation consumes the scored loan-level PDs and satellite coefficients produced by Components 1 and 2, generates the same scenario tables and figures, and can run either locally or on an Amazon EC2 Linux instance.

For the AWS deployment, the prepared Component 3 inputs are stored in Amazon S3 and the generated scenario outputs are written back to S3. EC2 accesses S3 through an IAM instance role rather than embedded credentials, with `boto3` providing the Python interface to S3. Components 1 and 2 remain the upstream notebook-based modeling workflow.
...
All tables and figures are generated by the project workflow and written to component-specific artifact folders. Components 1 and 2 are executed through notebooks; Component 3 can be executed either through its original notebook or through the refactored standalone Python scenario engine.
